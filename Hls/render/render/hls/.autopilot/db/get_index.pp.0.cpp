...
#pragma HLS ARRAY_PARTITION variable=lut complete dim=2
 ...
 ap_uint<2> center_x_mod3 = center_x % 3;
    ...
    VITIS_LOOP_19_1: for(ap_uint<4>i = 0; i < 3; i++) {
#pragma HLS PIPELINE II=1
 GET_pix_t neighbor_x = center_x + i - 1;
     ap_uint<2> x_mod3 =lut[center_x_mod3][i];
     ap_uint<7> internal_x = (neighbor_x-x_mod3) / 3;
        VITIS_LOOP_24_2: for(ap_uint<4> j = 0; j < 3; j++) {
#pragma HLS UNROLL

 ap_uint<4> num=i+j*3;
            GET_pix_t neighbor_y = center_y + j - 1;
